...
const {UserSchema, GroupsSchema, ScheduleSchema, LocationSchema} = require('./schemas');
...
// Add utility method to Mongoose model so we can create
// Mongoose object from a JSON object
LocationSchema.fromJSON = (obj) => {
    return new Location(normalizeObject(obj))
};
...
module.exports = ( async () => {
    ...
    return {
        ...
        addLocation: async function (latitude, longitude, type, name) {
            try{
                const u = LocationSchema.fromJSON({
                    latitude: latitude,
                    longitude: longitude,
                    type: type ? type : '',
                    name: name ? name : ''
